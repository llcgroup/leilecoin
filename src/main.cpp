...
#if defined(NDEBUG)
# error "Leilecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Leilecoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Leilecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Leilecoin: find block wanted by given orphan block
...
// Leilecoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Leilecoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Leilecoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Leilecoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Leilecoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Leilecoin: track money supply and mint amount info
...
// Leilecoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Leilecoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Leilecoin: compute stake entropy bit for stake modifier
    ...
    // Leilecoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Leilecoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Leilecoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Leilecoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Leilecoin-loadblk");
